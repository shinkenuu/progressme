...
public class OppDetailsGUIScript :  UserGui {
    ...
    {
        //IF is not a OppDetailGui action
        if ((int)action < 10 || (int)action > 19)
        {
            return;
        }
    ...
    protected override void FetchNextAction()
    {
        ...
        {
            case UserGuiAction.SETUP_OPPORTUNITY_DETAILS:
                StartCoroutine(FillInOppInfo());
                break;
        }
    }
    ...
    private void SetProcedure(GameManagerScript.Procedures selectedProcedure)
    {
        _selectedProcedure = selectedProcedure;

        switch(selectedProcedure)
        {
            case GameManagerScript.Procedures.OppAdvance:
                msgTxt.text = "Please select the process in which you want to advance this task to";
                DisplayAdjacentProcesses();
                btnNextGo.SetActive(false);
                break;
            case GameManagerScript.Procedures.OppTransfer:
                msgTxt.text = "Are you sure you want to transfer this task?";
                ClearPanel(processPanelTransform);
                btnNextGo.SetActive(true);
                break;
            case GameManagerScript.Procedures.OppEnd:
                msgTxt.text = "Are you sure you want to end this task?";
                ClearPanel(processPanelTransform);
                btnNextGo.SetActive(true);
                break;
        }
    ...
    public override void BtnActionNext()
    {
        GameMng.ScheduleProcedure(_selectedProcedure);
        GameMng._tempOpp = _detailedOpp;
        GameMng._tempProcess = _selectedProcess;

        base.BtnActionNext();       
    }

    public void BtnActionReport()
    {
        GameMng.ScheduleOppReport();
    }
